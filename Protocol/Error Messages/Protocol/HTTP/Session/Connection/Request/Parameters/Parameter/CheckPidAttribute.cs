namespace Skyline.DataMiner.CICD.Validators.Protocol.Tests.Protocol.HTTP.Session.Connection.Request.Parameters.Parameter.CheckPidAttribute
{
    ...
    internal static class Error
    {
        public static IValidationResult NonExistingId(IValidate test, IReadable referenceNode, IReadable positionNode, string pid, string sessionId, string connectionId)
        {
            return new ValidationResult
            {
                ...
                GroupDescription = "",
                Description = String.Format("Attribute '{0}' references a non-existing '{1}' with {2} '{3}'. {4} {5} '{6}'. {7} {8} '{9}'.", "Request/Parameters/Parameter@pid", "Param", "ID", pid, "HTTP Session", "ID", sessionId, "Connection", "ID", connectionId),
                HowToFix = "",
                ExampleCode = "",
                Details = "Use this attribute to specify the id of an existing parameter containing the value for this request parameter.",
                HasCodeFix = false,
        ...
        public static IValidationResult EmptyAttribute(IValidate test, IReadable referenceNode, IReadable positionNode, string sessionId, string connectionId)
        {
            return new ValidationResult
            {
                ...
                FixImpact = FixImpact.NonBreaking,
                GroupDescription = "",
                Description = String.Format("Empty attribute '{0}' in {1} '{2}'. {3} {4} '{5}'.", "Request/Parameters/Parameter@pid", "HTTP Session", sessionId, "Connection", "ID", connectionId),
                HowToFix = "",
                ExampleCode = "",
                Details = "Use this attribute to specify the id of an existing parameter containing the value for this request parameter.",
                HasCodeFix = false,
        ...
        public static IValidationResult InvalidAttribute(IValidate test, IReadable referenceNode, IReadable positionNode, string pidValue, string httpSessionId, string connectionId)
        {
            return new ValidationResult
            {
                ...
                FixImpact = FixImpact.NonBreaking,
                GroupDescription = "",
                Description = String.Format("Invalid value '{1}' in attribute '{0}'. {2} {4} '{3}'. {5} {7} '{6}'.", "Parameter@pid", pidValue, "HTTP Session", httpSessionId, "ID", "Connection", connectionId, "ID"),
                HowToFix = "",
                ExampleCode = "",
                Details = "Use this attribute to specify the id of an existing parameter containing the value for this request parameter.",
                HasCodeFix = false,
    ...
    public static class CheckId
    {
        ...
        /// </summary>
        public const uint CheckPidAttribute = 12;
    }
}