namespace SLDisValidator2.Tests.Protocol.Pairs.Pair.Content.Response.CheckResponseTag
{
    using System;
    ...
    internal class CheckResponseTag : IValidate, ICodeFix/*, ICompare*/
    {
        public List<IValidationResult> Validate(ValidatorContext context)
        {
            ...
            foreach (IPairsPair pair in context.EachPairWithValidId())
            {
                ...
                {
                    if (!(item is IPairsPairContentResponse response))
                    {
                        continue;
                    }
        ...
        public ICodeFixResult Fix(CodeFixContext context)
        {
            ...
            switch (context.Result.ErrorId)
            {
                case ErrorIds.UntrimmedTag:
                    if (!(context.Result.ReferenceNode is IPairsPairContentResponse readResponse))
                    {
                        result.Message = "context.Result.Node is not of type IPairsPairContentResponse";
                        break;
                    }
                    ...
                    {
                        var editResponse = pair.Content.Get(readResponse) as Skyline.DataMiner.CICD.Models.Protocol.Edit.PairsPairContentResponse;
                    ...
                    if (!result.Success)
                    {
                        result.Message = "Matching 'Content/Response' tag not found.";
                    }
