namespace Skyline.DataMiner.CICD.Validators.Protocol.Tests.Protocol.HTTP.Session.CheckUsernameAttribute
{
    ...
    internal class CheckUsernameAttribute : IValidate/*, ICodeFix, ICompare*/
    {
        public List<IValidationResult> Validate(ValidatorContext context)
        {
            ...
            foreach (IHTTPSession session in context.EachHttpSessionWithValidId())
            {
                ...
                (GenericStatus valueStatus, uint _) = GenericTests.CheckBasics<uint>(session.UserName.Value);
                ...
                if (!context.ProtocolModel.TryGetObjectByKey<IParamsParam>(Mappings.ParamsById, session.UserName.Value, out _))
                {
                    results.Add(Error.NonExistingId(this, session, session, session.UserName.Value, session.Id.RawValue));
                    continue;
                }
            }
