    ...
    internal class CheckNameAttribute : ICompare
    {
        public List<IValidationResult> Compare(MajorChangeCheckContext context)
        {
            ...
            foreach (var oldDve in oldDves)
            {
                ...
                {
                    results.Add(ErrorCompare.RemovedItem(oldDve, oldDve, oldDve.Name.Value, Convert.ToString(oldDve.TablePID.Value)));
                }
            }
    ...
    internal static class CompareHelper
    {
        public static void CompareWithNewDve(List<IValidationResult> results, IDVEsDVEProtocols newDves, IDVEsDVEProtocolsDVEProtocol oldDve)
        {
            ...
            if (newDve != null)
            {
                ...
                if (!oldDve.Name.Value.Equals(newDve.Name.Value))
                {
                    results.Add(ErrorCompare.UpdatedValue(newDve, newDve, oldDve.Name.Value, Convert.ToString(oldDve.TablePID.Value), newDve.Name.Value));
                }
            }
            else
            {
        ...
        private static IDVEsDVEProtocolsDVEProtocol FindMatchingDve(IDVEsDVEProtocols newDves, IDVEsDVEProtocolsDVEProtocol oldDve)
        {
            ...
            {
                uint? oldTableId = oldDve?.TablePID?.Value;
                uint? newTableId = newDve?.TablePID?.Value;
