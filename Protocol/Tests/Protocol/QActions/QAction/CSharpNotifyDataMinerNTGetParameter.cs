namespace Skyline.DataMiner.CICD.Validators.Protocol.Tests.Protocol.QActions.QAction.CSharpNotifyDataMinerNTGetParameter
{
    ...
    internal class QActionAnalyzer : QActionAnalyzerBase
    {
        ...
        {
            // protocol.NotifyDataMiner(73, [Array], null)

            if (!callingMethod.IsNotifyDataMiner(semanticModel, solution, 73))
            {
                return;
            }
            ...
            if (value.IsNumeric())
            {
                // Local GetParameter => People should use the protocol.GetParameter call instead.
                // We assume (Never assume!) that when people use the raw NotifyProtocol, it will be for remote elements.
                results.Add(Error.DeltIncompatible(test, qAction, qAction, qAction.Id.RawValue));
                return;
            }
