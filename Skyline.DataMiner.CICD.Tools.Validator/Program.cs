...
description: "Name of the results file. Note: Do not provide an extension, the extension is automatically added based on the results-output-formats option. Default: 'ValidatorResults_<protocolName>_<protocolVersion>'")
			{
				IsRequired = false
			};
...
getDefaultValue: () => 300000);
		...
		public static async Task<int> ValidateProtocolSolution(string solutionFilePath, string validatorResultsOutputDirectory, string validatorResultsFileName, string[] outputFormats, bool includeSuppressed, bool performBuild, int buildTimeout)
		{
			...
			if (validatorResultsFileName != null)
			{
				resultsFileName = validatorResultsFileName;
			}
			else
			{
				resultsFileName = $"ValidatorResults_{validatorResults.Protocol}_{validatorResults.Version}";
			}
			sw.Stop();
			...
			{
				resultWriters.Add(new ResultWriterXml(Path.Combine(validatorResultsOutputDirectory, $"{resultsFileName}.xml")));
			}

			if (outputFormats.FirstOrDefault(f => String.Equals(f, "JSON", StringComparison.OrdinalIgnoreCase)) != null)
			{
				resultWriters.Add(new ResultWriterJson(Path.Combine(validatorResultsOutputDirectory, $"{resultsFileName}.json")));
			}
