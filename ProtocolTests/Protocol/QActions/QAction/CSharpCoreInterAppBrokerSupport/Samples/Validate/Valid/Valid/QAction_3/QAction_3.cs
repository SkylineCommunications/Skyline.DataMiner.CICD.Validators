	...
	public static void Run(SLProtocolExt protocol)
	{
		try
		{
			...
			foreach (var message in receivedCall.Messages)
			{
				...
				{
					var replyToReply = message.Reply(protocol.SLNet.RawConnection, returnMessage,TimeSpan.FromSeconds(5), SerializerFactory.CreateInterAppSerializer(new List<Type> { }));
				}
			}
		}
		catch (Exception ex)
